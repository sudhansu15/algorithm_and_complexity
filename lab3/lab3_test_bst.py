...
class BSTTestCase(unittest.TestCase):
    ...
    def test_selfWritten(self):

        # when the binary search tree is empty 
        bbst = BinarySearchTree()
        self.assertEqual(bbst.search(10),False)

        # deleting empty BSTS
        self.assertEqual(bbst.remove(10),None)
        
        #deleting root node
        self.bst.remove(10)
        self.assertEqual(self.bst.size(), 7)
        self.assertListEqual(self.bst.inorder_walk(), [1, 5, 8, 30, 40, 45, 52])
        self.assertListEqual(self.bst.preorder_walk(), [8, 5, 1, 52, 40, 30, 45])
        
        # deleting key from BST that doesnt exist
        self.assertEqual(bbst.remove(99),None)
